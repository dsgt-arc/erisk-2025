...
from erisk.spark import spark_resource

from erisk.etl.embedding.ml import WrappedSentenceTransformer
...
class ProcessSentenceTransformer(luigi.Task):
    ...
    primary_key = luigi.Parameter(default="DOCNO")
    ...
    def run(self):
        ...
        with spark_resource(**kwargs) as spark:
            df_raw = spark.read.parquet(self.input_path)

            # Preprocess self-referential posts
            df = PreprocessSelfReferentialPosts(df_raw)
            ...
            df = (
                df.withColumn(
                    "sample_id", F.crc32(F.col(self.primary_key)) % self.num_sample_ids
                )
